    ...
    public function getCardsProperty(): array
{
    ...
    /** @var int|null $kasSettingId */
    $kasSettingId = $this->kasSettingId;

    $ringkasan = app(KasService::class)->getRingkasan($tahun);

    $totalAnggota = User::role(['pengurus', 'bendahara', 'sekretaris', 'ketua'])->count();

    $sudahBayarBulanIni = 0;
    if ($kasSettingId) {
        $sudahBayarBulanIni = PembayaranKas::where('kas_setting_id', $kasSettingId)
