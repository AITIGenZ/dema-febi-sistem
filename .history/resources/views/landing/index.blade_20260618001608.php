    ...
    <style>
        ...
        * {
        ...
        .speech-front .speech-text-content {
            grid-column: 2;
            /* Memaksa teks hanya mengisi kolom kedua */
        }
        ...
        .event-card:hover {
            ...
            box-shadow: 0 14px 36px rgba(26, 122, 122, 0.14);
        }
        ...
        .event-tag {
            font-size: 11px;
            color: var(--teal);
            border: 1.5px solid rgba(26, 122, 122, 0.3);
            padding: 3px 12px;
            border-radius: 50px;
            font-weight: 600;
        }
        ...
        #kalender-container {
            background: white;
            border-radius: 20px;
            border: 1px solid rgba(26, 122, 122, .1);
            padding: 24px;
...
<body>
                    ...
                    <div class="speech-card-inner">
                        ...
                        <div class="speech-front">
                            ...
                            <div class="speech-text-content">
                                <p class="speech-quote-text">"Bersama membangun FEBI yang Berkarakter dan Berprestasi."
                                </p>
                                <p class="speech-body-text">Assalamu'alaikum Wr. Wb. Selamat datang di SIMA DEMA FEBI —
                                    Semoga Platform ini menjadi Jembatan Komunikasi yang efektif antara Pengurus dan
                                    seluruh Civitas FEBI.</p>
                                <hr class="speech-hr">
                        ...
                        <div class="speech-back">
                            <div class="speech-text-content">
                                ...
                                <p class="speech-body-text">Mewujudkan DEMA FEBI yang harmonis dalam kepengurusan,aktif
                                    mengabdi kepada masyarakat,peka terhadap perkembangan informasi,serta menjunjung
                                    tinggi sportifitas dan profesionalitas </p>
                                <br>
                                <p class="speech-quote-text">Misi</p>
                                <p class="speech-body-text">1. Membudayakan komunikasi terbuka antar sesama ormawa febi
                                    terhadap permasalahan dan menjadi agen solutif untuk suatu perubahan</p>
                                <p class="speech-body-text">2. Mewujudkan program febi yg peduli dengan aksi sosial dan
                                    responsif terhadap isu masyarakat sekitar</p>
                                <p class="speech-body-text">3. Menciptakan hubungan internal ormawa febi yg saling
                                    menghargai dan dewasa sebagai individu yg profesional</p>
                                <hr class="speech-hr">
                            </div>
                        </div>
                    ...
                    <a href="{{ route('kegiatan.detail', $item->id) }}" class="event-card">
                        ...
                        <div class="event-body">
                            ...
                            <div class="event-desc">
                                {{ Str::limit($item->deskripsi, 140) ?? 'Kegiatan DEMA FEBI UIN Mahmud Yunus Batusangkar.' }}
                            </div>
                    ...
                    @foreach($kegiatan->take(3) as $item)
                        <a href="{{ route('kegiatan.detail', $item->id) }}" class="news-card">
                            <div class="news-thumb">
                                <div
                                    style="width:100%;height:100%;background:linear-gradient(160deg,{{ ['#1a9a9a', '#2563eb', '#7c3aed'][($loop->index % 3)] }},{{ ['#0f6060', '#1d4ed8', '#5b21b6'][($loop->index % 3)] }});">
                                </div>
                            ...
                            <div class="news-body">
                                ...
                                <div class="news-excerpt">
                                    {{ Str::limit($item->deskripsi, 110) ?? 'Kegiatan DEMA FEBI UIN Mahmud Yunus Batusangkar.' }}
                                </div>
                            </div>
                        </a>
                    @endforeach
    ...
    {{-- ===== KALENDER ===== --}}
    <section id="kalender" style="background:#0f5858; padding:64px 24px; font-family:'Plus Jakarta Sans',sans-serif;">
        <div style="max-width:720px; margin:0 auto;">

            <span
                style="color:#1a9090;font-size:10px;font-weight:800;letter-spacing:.18em;text-transform:uppercase;display:block;margin-bottom:10px;text-align:center;">Agenda</span>
            <h2 style="color:#f5f0e8;font-size:28px;font-weight:800;text-align:center;margin-bottom:6px;">Kalender
                Program Kerja</h2>
            <p style="color:rgba(245,240,232,0.5);font-size:13px;text-align:center;margin-bottom:36px;">Jadwal dan
                agenda kegiatan DEMA FEBI sepanjang tahun</p>

            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
                <button id="cal-prev"
                    style="background:rgba(26,144,144,0.15);border:1.5px solid rgba(26,144,144,0.3);color:#1a9090;width:38px;height:38px;border-radius:50%;font-size:20px;cursor:pointer;">&#8249;</button>
                <div style="text-align:center;">
                    <div id="cal-month"
                        style="color:#1a9090;font-size:22px;font-weight:900;text-transform:uppercase;letter-spacing:.06em;">
                    </div>
                    <div id="cal-year" style="color:#f5f0e8;font-size:15px;font-weight:700;margin-top:2px;"></div>
                </div>
                <button id="cal-next"
                    style="background:rgba(26,144,144,0.15);border:1.5px solid rgba(26,144,144,0.3);color:#1a9090;width:38px;height:38px;border-radius:50%;font-size:20px;cursor:pointer;">&#8250;</button>
            </div>
            ...
            <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:4px;margin-bottom:6px;">
                @foreach(['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'] as $h)
                    <div
                        style="text-align:center;color:#1a9090;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.08em;padding:6px 0;">
                        {{ $h }}</div>
                @endforeach
            </div>
            ...
            <div id="cal-popup"
                style="display:none;margin-top:16px;background:#eee8d8;border-radius:16px;padding:20px 22px;border:1px solid rgba(26,122,122,0.2);">
                <button id="cal-close"
                    style="float:right;background:none;border:none;color:#7a9090;cursor:pointer;font-size:18px;">&#10005;</button>
                <div
                    style="color:#1a7a7a;font-size:10px;font-weight:800;letter-spacing:.14em;text-transform:uppercase;margin-bottom:12px;">
                    Kegiatan</div>
    ...
    <script>
        (function () {
            ...
            let cur = new Date(), allEvents = [];

            fetch('/kalender-data').then(r => r.json()).then(d => { allEvents = d.data || d; render(); }).catch(() => render());
            ...
            function render() {
                ...
                for (let i = firstDay - 1; i >= 0; i--) addCell(grid, prevDim - i, true, false, []);
                for (let d = 1; d <= dim; d++) {
                    const ds = y + '-' + String(m + 1).padStart(2, '0') + '-' + String(d).padStart(2, '0');
                    const isToday = today.getFullYear() === y && today.getMonth() === m && today.getDate() === d;
                    const evs = allEvents.filter(e => e.start && e.start.startsWith(ds));
                    addCell(grid, d, false, isToday, evs);
                }
                const total = Math.ceil((firstDay + dim) / 7) * 7;
                for (let d = 1; d <= total - firstDay - dim; d++) addCell(grid, d, true, false, []);
                document.getElementById('cal-popup').style.display = 'none';
            }

            function addCell(grid, day, isOther, isToday, evs) {
                const cell = document.createElement('div');
                cell.style.cssText = 'min-height:62px;background:' + (isToday ? 'rgba(10,56,56,0.9)' : 'rgba(10,40,40,0.6)') + ';border-radius:10px;padding:8px;display:flex;flex-direction:column;border:' + (isToday ? '1.5px solid #1a9090' : '1px solid rgba(26,122,122,0.1)') + ';opacity:' + (isOther ? '.28' : '1') + ';cursor:' + (evs.length > 0 ? 'pointer' : 'default') + ';transition:background .15s;';
                if (!isOther) {
                    cell.onmouseenter = function () { this.style.background = 'rgba(26,80,80,0.8)'; };
                    cell.onmouseleave = function () { this.style.background = isToday ? 'rgba(10,56,56,0.9)' : 'rgba(10,40,40,0.6)'; };
                }
                const num = document.createElement('div');
                num.style.cssText = 'font-size:14px;font-weight:700;color:' + (isToday ? '#1a9090' : '#f5f0e8') + ';line-height:1;';
                num.textContent = day;
                cell.appendChild(num);
                if (evs.length > 0) {
                    const dots = document.createElement('div');
                    dots.style.cssText = 'margin-top:auto;display:flex;gap:3px;flex-wrap:wrap;padding-top:4px;';
                    evs.slice(0, 3).forEach(() => { const d = document.createElement('span'); d.style.cssText = 'width:6px;height:6px;border-radius:50%;background:#1a9090;display:inline-block;'; dots.appendChild(d); });
                    cell.appendChild(dots);
                    cell.addEventListener('click', () => showEvs(evs));
                }
                grid.appendChild(cell);
            }

            function showEvs(evs) {
                const c = document.getElementById('cal-popup-content');
                c.innerHTML = '';
                evs.forEach(e => {
                    const item = document.createElement('div');
                    item.style.cssText = 'display:flex;align-items:flex-start;gap:12px;padding:12px 14px;background:white;border-radius:10px;margin-bottom:8px;border:1px solid rgba(26,122,122,0.1);';
                    item.innerHTML = '<span style="width:8px;height:8px;border-radius:50%;background:#1a7a7a;margin-top:4px;flex-shrink:0;display:inline-block;"></span><div><p style="color:#1a2a2a;font-weight:700;font-size:13px;margin:0 0 2px;">' + e.title + '</p>' + (e.description ? '<p style="color:#7a9090;font-size:12px;margin:0;">' + e.description + '</p>' : '') + '</div>';
                    c.appendChild(item);
                });
            ...
            document.getElementById('cal-close').addEventListener('click', () => { document.getElementById('cal-popup').style.display = 'none'; });
        })();
    </script>
    {{-- ===== FOOTER ===== --}}
    <footer class="footer">
                ...
                <div>
                    ...
                    <div class="ft-socials">
                        ...
                        <a href="#" title="Instagram">
                            <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor">
                                <path
                                    d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069" />
                            </svg>
                        </a>
                    </div>
                </div>
